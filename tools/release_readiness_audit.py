...
SKIP_DIRS = {".git", ".godot", "build", "generated", "vendor", "third_party"}
TEXT_SUFFIXES = {".gd", ".tscn", ".tres", ".cfg", ".md", ".txt", ".json", ".yml", ".yaml", ".html", ".js", ".css", ".gdshader", ".py"}
FORBIDDEN_FILE_SUFFIXES = {".p12", ".pfx", ".mobileprovision", ".keystore", ".jks"}
SECRET_PATTERNS = {
...
def read_text(path):
    if path.suffix.lower() not in TEXT_SUFFIXES and path.name not in {"VERSION", ".gitignore"}: return None
...
def main():
    ...
    for path in iter_repo_files():
        relative=rel(path); suffix=path.suffix.lower()
        if suffix in FORBIDDEN_FILE_SUFFIXES: failures.append(f"forbidden signing/credential file tracked: {relative}")
        text=read_text(path)
        if text is None: continue
        for label, pattern in SECRET_PATTERNS.items():
            if pattern.search(text): failures.append(f"{label} pattern detected in {relative}")
        if relative not in ALLOWED_ABSOLUTE_PATH_FILES:
            for label, pattern in ABSOLUTE_PATH_PATTERNS.items():
                if pattern.search(text): failures.append(f"{label} detected in {relative}")
        ...
        if suffix in {".gd", ".tscn", ".tres", ".gdshader"}:
            for marker in ("TODO", "FIXME", "PLACEHOLDER"):
                if marker in text: warnings.append(f"runtime marker {marker} remains in {relative}")
    if failures:
    ...
    if warnings:
        ...
        for item in warnings: print(f"WARN: {item}")
    else: print("No runtime TODO/FIXME/PLACEHOLDER markers detected.")
    return 0
